...
app.post('/create-task/process',async (req,res)=>{
    ...
    try {
        const completion = openai.chat.completions.create({
            messages:[{
            role: "system", 
            content: "\n\nTask description: "+Task.Task_Description + "\nTask StartDate: " + Task.Start_Date + "\nDays until due:" + Task.End_Date + "\nMax Time Per Day"+ Task.Max_Time_Per_Day+"\nTake the role of a helpful AI scheduling assistant. Your job is to take tasks and break them down into subgoals taking the user's start date, end date,max time they are willing to work on it in a day, and task description into account and curating a schedule that will help them complete their task. keep in mind that the human brain can only focus for 1.5 hours on a given task effectively. leave time for breaks if they have to do multiple tasks in a day. Categorize each task into either a left-brain activity or right-brain activity then schedule such that, as much as possible that the left brain activities balance out with the right brain activities so that variety is maximized over the course of the day or week. Only schedule tasks between the normal working hours of the day unless a deadline has to be met. Write a proposed schedule in in ICS format. minimize colen use, do not put any refrence to date or day in the description. ",
            }],
            model:"gpt-4",
        });
    ...
    res.set({'Content-disposition':'attachment; filename=' + "task.ics",'Content-Type':'text/plain'});
    readStream.pipe(res);
